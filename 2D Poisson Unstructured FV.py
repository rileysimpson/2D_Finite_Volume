...
gamma = 1
# solver properties
norm_res_target = 0.01  # normalized residual target
num_n2 = 10  # number of inner iterations per outer iteration
# display options
pre_info = False
post_info = False
plot_contour = True
plot_mesh = False
plot_residual = False
plot_3D_residual = False
plot_cell = False
plot_vertex = False
n1n2_per_print = 20
# display settings
scale_unit_vectors = 1 / 8  # unit vector scaling
contour_res = 100
# ===end input===
...
c_x = np.zeros(num_cell)  # x coordinate of cell centroid
c_y = np.zeros(num_cell)  # y coordinate of cell centroid
f_x = np.zeros(num_face)  # x coordinate of face midpoint
f_y = np.zeros(num_face)  # y coordinate of face midpoint
# declare face property arrays
fn_x = np.zeros(num_face)  # x component of face normal (global face index)
fn_y = np.zeros(num_face)  # y component of face normal (global face index)
ft_x = np.zeros(num_face)  # x component of face tangent (global face index)
ft_y = np.zeros(num_face)  # y component of face tangent (global face index)
fn_sign = np.zeros((num_cell, 3))  # sign modifier (-1 for inward pointing normal)
area = np.zeros(num_face)  # area of face
delta = np.zeros(num_face)  # volume of cell
# declare volume property arrays
vol = np.zeros(num_cell)  # volume of cell
# declare connectivity arrays
c_f = np.full((num_cell, 3), -1, dtype=int)  # cell to face connectivity (cell index, local face index)
f_c = np.zeros((num_face, 2), dtype=int)  # face to cell connectivity (face index, local cell index)
f_v = np.zeros((num_face, 2), dtype=int)  # face to vertex connectivity (face index, local vertex index)
c_v = np.full((num_cell, 3), -1,
              dtype=int)  # face to vertex connectivity, global vertex index = c_v(cell index, local vertex index)
bf_f = np.zeros(num_bface, dtype=int)  # boundary face index to global face index
f_bf = np.zeros(num_face, dtype=int)  # global face index to boundary face index
# declare flag arrays
...
wtsum_c_v = np.zeros(num_vertex)  # cell-to-vertex interpolation weight sum
...
wt_check = np.zeros(num_vertex)  # right-hand side
...
if plot_contour:
    ...
    ax.tricontourf(v_x, v_y, v_phi, contour_res, cmap='viridis')  # choose 20 contour levels, just to show how good its interpolation is
    for i in range(0, num_face):
        if bf_flag[i] == 1:
            v1 = f_v[i, 0]
            v2 = f_v[i, 1]
